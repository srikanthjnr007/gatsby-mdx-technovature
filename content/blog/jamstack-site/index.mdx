...
When we first wrote this post, the [JAMstack](https://jamstack.org/) ecosystem was still in its infancy.

![jamstack? sure](The%20JAMstack%20in%202020%20Why%20(and%20How)%20to%20Get%20Started%20-%20Snipcart_files/giphy.gif)

Boy, are we grateful today to have witnessed firsthand the rise of this now vibrant community. Its fair to say that we've played our part in this movement crafting nearly fifty JAMstack-related [blog posts](https://snipcart.com/blog/categories/jamstack) & [Github repositories](https://github.com/snipcart) with actionable, live demos.

The difference today is that this kind of content pops up everywhere! And donâ€™t get me wrong, weâ€™re all for it. It means the community is in good shape and growing stronger every year.

We plan to keep up providing [valuable resources](https://snipcart.com/jamstack-ecommerce-podcast) to new & seasoned JAMstack developers alike in 2020. It starts today with this update to our introduction post and what we think the year ahead has in store for devs.
...
The JAMstack isnâ€™t a specific set of tools, but a new, modern way of building websites & apps. Where does â€œJ-A-Mâ€� come from, you may ask? From these three core concepts:
...
We became aware that traditional CMSs (WordPress, Drupal) were just trying to doâ€¦ **too much**.

We realized how cumbersome they were. How opinionated. These user-friendly admins started feeling less friendly.

Thanks to modern browsers, static site generators, [CDNs](https://en.wikipedia.org/wiki/Content_delivery_network), and APIs, weâ€™re now seeing web developers transition from dynamic server-side applications to modular, client-side stacks.
...
But it didnâ€™t come to life with just the snap of a finger. Like I said at the beginning, itâ€™s been a few years in the making.

### 1.2 Timeline

Letâ€™s take a quick journey back in time with a year-by-year view of the rise of the JAMstack. It should clarify where itâ€™s at right now and where itâ€™s going.

â†' **2015**: Static sites are slowly making a comeback from the ruins of the webâ€™s early years. The first CMS-deniers are making them â€œcoolâ€� again.

â†' **2016**: As you would expect, backlash occurs. Static sites arenâ€™t cool at allâ€"they lack too many features to build anything other than blogs. In the meantime though, a small group of developers is coining the â€œJAMstackâ€� and slowly promoting its principles in modern dev circles.

â†' **2017**: The year JAMstack really comes to life, for a somewhat niche community. Static sites aren't â€œstaticâ€� anymore. This modern web revolution gives you all the features you need to build â€œhyper-dynamicâ€� sites & apps. Sequoia Capital, Mailchimp & Red Bull are a few of the first big enterprises to build JAMstack projects.

â†' **2018**: Hereâ€™s a phrase we start to hear a lot: â€œJust discovered the JAMstack and, oh my God, it's amazing!â€� Yup, the paradigm makes a mainstream breakthrough with more & more people talking about it. Substantial funding is announced for tools like Gatsby, Netlify, Contentful, etc. The first [JAMstack_conf](https://jamstackconf.com/) takes place.

â†' **2019**: The year of maturity & accessibility. The JAMstack isnâ€™t a niche community anymore. Most frontend developers hear about it and many start looking into it. With the likes of [Stackbit](https://www.stackbit.com/), the JAMstack opens its doors to [less technical users](https://snipcart.com/blog/manage-jamstack-websites-stackbit). The rise of [serverless functions](https://snipcart.com/blog/why-serverless-example) is also huge for bringing more backend functionalities to frontend-centric projects.

â†' **2020:** With a new decade on the horizon, thereâ€™s nothing indicating a decline in the JAMstackâ€™s adoption. If services and APIs in the ecosystem continue to prove viable and profitable, weâ€™ll see more important players embracing the idea, increasing adoption even more. The New Dynamic brought legit concerns about having **too much** options in the ecosystem in its [first newsletter of the year](https://www.thenewdynamic.org/article/2020-11-01-jamstack-update/). Food for thought, no doubt. Otherwise, Iâ€™d say the future looks bright for JAMstackers! More insights for 2020 coming in the last section of this post.
...
This graphic is a very stripped-down view of whatâ€™s actually going on under the hood. Hereâ€™s what it looks like when you zoom in:
...
If youâ€™re new to this whole idea of the JAMstack, you might be a little overwhelmed at this point. Knowing where to start is sometimes the hardest part.
...
Starting from scratch is the best way to build a â€œpureâ€� JAMstack. This way you can really choose all the necessary pieces to create what you want. As I explained earlier, building and hosting are decoupled. The first thing youâ€™ll want to decide is **what to use to build your site/app**.

The modern frontend ecosystem may make your life hard here. Not because of a lack of relevant tools. Actually, itâ€™s just the opposite! There are hundreds of possibilities, even if some clearly stand out.
...
Once youâ€™ve built your site/app, it needs to live somewhere. Most developers turn to [GitHub pages](https://github.com/), [Netlify](https://www.netlify.com/) or [Zeit](https://zeit.co/) to host their code.

**So hereâ€™s a few use case examples of what your new stack could look like:**
...
> Iâ€™ve myself migrated a personal blog from Medium to a static site powered by Gatsby & Netlify CMS. I described that process in [this post](https://snipcart.com/blog/jamstack-templates-example).
...
Because itâ€™s just not their lingo.
...
â€œBy ditching the big web server, the databases, the plugins, and the constant maintenance, weâ€™ll save you X thousands of dollars a year.â€�

Hard to argue with that.

### 3.2 Refuting common objections

The JAMstack isnâ€™t bulletproof yet. While discussing a switch to it, more tech-savvy clients or developers may bring up some (more or less) legitimate concerns. Here are the two objections we used to see frequently in online threads that could still come up in conversation:

**1) A JAMstack set up is nice, but I need to handle dynamic features on my site.** Iâ€™ll repeat it one last time: the JAMstack sites built today arenâ€™t static anymore. Theyâ€™re hyper-dynamic. So regarding the presumed lack of server-side features, you should refer to the section about added functionalities & third-party APIs.
...
â€¦ And [our blog](https://snipcart.com/blog/categories/jamstack), obviously! Make sure not to miss anything by [subscribing to our newsletter](https://us5.list-manage.com/subscribe?u=c019ca88eb8179b7ffc41b12c&id=3e16e05ea2). Over the years we also submitted a few of our own articles to the [awesome-jamstack](https://github.com/automata/awesome-jamstack) GitHub repo which is also an excellent community hub. Oh, lastly, you should follow #jamstack on Twitter!
...
At Snipcart, we are always open to relevant guest posting from devs with killer content to share! Let us know if youâ€™re interested.

## 5. Closing thoughts

Feel free to start the conversation right here in the comments. Whereâ€™s your head at when it comes to the JAMstack? Are you all in? Still having doubts? What are you looking for the most in the ecosystem in 2020? Weâ€™d love to read your opinion.

Iâ€™ll end this (long) post on a more â€œpersonalâ€� note. As a team and a business, weâ€™re still 100% behind the JAMstack and constantly in awe of the expansion it has been through in a short amount of time. Here are rapid-fire thoughts coming from the Snipcart team members:

* Looking forward to new (or improvement of existing) tools to manage medium to large JAMstack projectsâ€"SaaS subscriptions management, top-level observability of a whole setup, etc.
* With the increasing regard to building **fast** web experiences, a wider adoption of the PWA movement is to anticipate. We think itâ€™s great news.
* I mentioned it earlier, but the release of incremental builds in static site development in 2020 would be game-changing. Looking at you Gatsby! ðŸ'€
* Going more mainstream will mean a significant bump in [marketers/SEOs adoption](https://kymellis.co/jamstack-marketing-headless-cms/).
* It should also lead to more awareness in the WP ecosystem. The likes of [WP2Static](https://wordpress.org/plugins/static-html-output-plugin/),[Strattic](https://www.strattic.com/) and [Shifter](https://www.getshifter.io/) are already leading the way.
* And, finally, a stronger focus on bundling tools like Stackbit or Netlify can only help to open the JAMstack to a wider audience and boost accessibility.

Let's hope 2020 meets our expectations! In the meantime, happy coding to you all. :)
